...
Executable today: DISCOVER through DIAGNOSE, SCORE, PLAN and VALIDATE run in-process for a single target (`ope audit`), and MONITOR is served by the local run history, which compares each run against the previous one for the same target. PLAN is produced by `planner.build_remediation_plan()`: it orders the diagnosed failures by dependency impact (fix the root-cause module that unblocks the most downstream modules first), attaches each module's evidence-backed findings, and never invents remediation. IMPLEMENT stays human-owned — the plan says what to fix and in what order, never applies changes — and scheduled multi-target monitoring is not implemented. See `automation/audit-runner-v1.md` for the step-by-step status and the README for per-module check coverage.
