...
from .integrations.osv import detect_dependencies, scan_from_env
...
def audit(url: str, timeout: int = 15, fetch_subresources: bool = True, browser: bool = False, browser_timeout: int = 30, browser_profiles: list[str] | None = None) -> dict[str, Any]:
    ...
    client_dependencies = detect_dependencies(p.script_srcs)  # local detection, always
    dependency_scan = scan_from_env(p.script_srcs)            # OSV network scan, opt-in
